    ...
    cy.get("ul a").each(($link, index) => {
      ...
      if (href) {
        ...
        if (href.includes("/register")) {
          // Intercept fake registration API response
          ...
          // Wait for the response and then set cookie
          cy.wait("@registerUser").then(() => {
            cy.setCookie("accessToken", "mock-access-token");
            cy.getCookie("accessToken").should("exist"); // Debugging step
            cy.visit("localhost:3000");
          });

          // Visit profile page and get and update user
          cy.visit("/profile");
          ...
          //Visit Add blog page and get and update blog
          cy.visit("/add-blog");
          ...
          // Wait for login response and then set cookie
          cy.wait("@login").then(() => {
            cy.setCookie("accessToken", "mock-access-token");
        ...
        // Go back to home before testing the next link
