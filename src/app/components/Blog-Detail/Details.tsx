...
const findBlogByTitle = (title: string) => {
  return BlogPostsData.find(blogPost => blogPost.title.toLowerCase() === title.toLowerCase());
}
...
const Details = ({ params }: BlogPostProps) => {
  ...
  return (
    <div>
      ...
      {blog ? (
        <>
        <Header title={blog.title} category={blog.category} author={blog.author} date={blog.date}/>
        ...
        <div className="px-5 sm:px-8 md:px-16 pt-16 lg:px-[21rem] pb-8 lg:pb-16">
          <div className="mt-8">
            ...
            {/* <div className="mb-8">
              ...
              <p className="text-gray-500">Additional blog details can go here.</p>
            </div> */}
          </div>
        </div>
        </>
